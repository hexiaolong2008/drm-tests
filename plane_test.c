...
struct test_plane {
	const struct draw_format *format;
...
static bool parse_format(const char *str, const struct draw_format **format)
{
	if (strlen(str) == 4) {
		const struct draw_format *draw_format = get_draw_format(*(uint32_t *)str);
		if (draw_format) {
			*format = draw_format;
			return true;
		}
		...
		for (size_t format_index = 0;
		     ...
		     format_index++) {
			const struct draw_format *draw_format = &g_draw_formats[format_index];
			if (!strcmp(str, draw_format->name)) {
				*format = draw_format;
				return true;
			}
		}
...
int main(int argc, char **argv)
{
	...
	for (size_t test_plane_index = 0; test_plane_index < test_planes_count;
		...
		if (!tp->format)
			tp->format = &g_draw_formats[0];

		test_planes_formats[test_plane_index] = tp->format->pixel_format;
		...
		printf("Creating buffer %ux%u %s (%4.4s)\n", tp->bo_w, tp->bo_h, tp->format->name,
		       (char *)&tp->format->pixel_format);
		tp->bo = gbm_bo_create(gbm, tp->bo_w, tp->bo_h, tp->format->pixel_format,
				       GBM_BO_USE_SCANOUT | GBM_BO_USE_LINEAR);
		if (!tp->bo) {
		...
		if (!draw_pattern(tp->bo, tp->format)) {
